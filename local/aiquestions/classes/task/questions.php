...
class questions extends \core\task\adhoc_task {
    ...
    public function execute() {
        ...
        while (!$created && $i < $numoftries) {
            ...
            // If we got any text back, try to create questions from it, even if the
            // strict GIFT validator is not completely happy. The GIFT importer itself
            // will skip invalid questions.
            if (!empty(trim($questions->text ?? ''))) {
                // Create the questions, return an array of objects of the created questions.
                ...
                if ($created) {
                    ...
                    // No questions could be created from the returned text.
                    $error = 'No valid questions could be created from returned text.';
                }
            }
            $i++;
        }
